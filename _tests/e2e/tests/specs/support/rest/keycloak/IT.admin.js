...
class ItAdmin {
    ...
    createSimpleUser(user) {
        const url = 'http://servicejava.edge.stage.ext.phx2.redhat.com/svcrest/simpleuser/suser/thirdparty?system=rhd';
        try {
     ...
     */
    createFullUser(user) {
        const url = 'http://servicejava.edge.stage.ext.phx2.redhat.com/svcrest/user/v3/createwithoutwelcome';
        try {
    ...
    _changePassword(email, password) {
        ...
        const url = `http://servicejava.edge.stage.ext.phx2.redhat.com/svcrest/simpleuser/pwd/login=${encodedEmail}?password=${password}`;
        try {
    ...
    findUserByEmail(email) {
        let res;
        const encodedEmail = qs.escape(email);
        const url = `http://servicejava.edge.stage.ext.phx2.redhat.com/svcrest/user/v3/email=${encodedEmail}`;
        try {
     ...
     */
    disableUser(user) {
        const userProfile = this.findUserByEmail(user.email);
        const url = 'http://servicejava.edge.stage.ext.phx2.redhat.com/svcrest/user/v3/update';
        const res = wait(request('POST', url, {
            headers: {
                "Content-Type": "application/json",
            },
            json: {
