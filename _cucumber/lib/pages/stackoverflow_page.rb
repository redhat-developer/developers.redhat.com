...
# this is the page class that contains all elements and common methods related to the SO page.
class StackOverflowPage < StandardisedSearch
  ...
  def question_with_answer
    find_answer { query_questions('with')[1] }
    @element = query_questions('with')[0]
  end

  def question_without_answer
    find_answer { query_questions('without')[1] }
    ...
    questions.each do |q|
      ...
      parent_li.each do |t|
        ...
        if answer == 'with'
          ...
          return @question_element, true unless child_elements.include? 'callout qtn-answer display-answer'
        end
      end
    end
  end
