Feature: DEVELOPER-1934 - Resources update: Implementation
  ...
  Scenario Outline: Clicking on the remove filter icon should replay results without filter.
    ...
    When I uncheck the "<filter type>" filter
    ...
    Examples: Filter types and results.
      ...
      | Code Artifact                    |
      | Get Started                      |
      | Knowledgebase Article / Solution |
      | Video                            |
  ...
  Scenario: Quoted keywords should filter by items containing the full search string.
    Given I am on the Resources page
    When I enter "Container Development Kit" into the Keyword's box
    Then the results displayed should contain "Container Development Kit"
  ...
  Scenario: Filter by product should display results related to that product
    Given I am on the Resources page
    When select "Red Hat Enterprise Linux" from the product filter
    Then the results should be updated
    And some of the results should contain a "RHEL" tag
