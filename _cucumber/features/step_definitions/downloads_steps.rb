...
Then(/^the pdf download should initiate$/) do
  on PromotionsPage do |page|
    if ENV['DEVICE']
      puts "PDF file was not automatically downloaded for #{$rhd_driver}, downloaded using the fallback link."
...
Then(/^the download should initiate$/) do
  ...
  download = Dir[File.join($download_directory, '**', '*')].count { |file| File.file?(file) }
  download.should == 1
end
