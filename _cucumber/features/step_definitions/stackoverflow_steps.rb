...
Then(/^each question should link to the relevant question on Stack Overflow$/) do
  on StackOverflowPage do |page|
    page.question_links.each { |link| expect(link.href).to include 'stackoverflow.com/questions/' }
  end
end

When(/^a question contains an answer$/) do
  @current_page.question_with_answer
end

When(/^a question does not contain an answer$/) do
  @current_page.question_without_answer
end

When(/^I (should|should not) see a "([^"]*)" section$/) do |negate, _arg|
  if negate == 'should'
...
Then(/^the results should be updated containing questions relating to "([^"]*)"$/) do |_arg|
  on StackOverflowPage do |page|
    updated_qtn_content = []
