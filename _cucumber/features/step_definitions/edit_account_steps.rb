And(/^the following newly registered details should be added to my profile:$/) do |table|
  on EditAccountPage do |page|
    table.raw.each do |row|
      ...
      case element
        when 'Username'
          expect(page.username.value).to eq $site_user[:email].gsub('@redhat.com', '').gsub('+', '-').gsub('_', '')
        when 'Email'
          expect(page.email.value).to eq $site_user[:email]
        when 'First Name'
          expect(page.first_name.value.downcase).to eq $site_user[:first_name].downcase
        when 'Last name'
