...
Then(/^a list of products available for download$/) do
  expect(@page.downloads).to have_product_downloads :count => @products_with_downloads.size
  @page.downloads.available_downloads.should =~ @products_with_downloads
end

Then(/^the 'Download Latest' links for available products$/) do
  expect(@page.downloads).to have_download_latest_links :count => @products_with_downloads.size
end
