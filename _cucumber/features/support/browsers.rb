class Browsers
  ...
  def setup(browser_name)
    case browser_name
      ...
      when 'android'
        browser = appium_driver(browser_name)
      when 'iphone'
        browser = appium_driver(browser_name)
      else
  ...
  def appium_driver(device)
    case device
      when 'iphone'
        test_device = ENV['DEVICE'] || 'iPhone 6'
        caps = { platformName: 'ios', deviceName: test_device, browserName: 'Safari', platformVersion: '9.2'}
      when 'android'
        test_device = ENV['DEVICE'] || 'test_device'
        caps = { platformName: 'Android', platformVersion: '5.1', deviceName: test_device, browserName: 'Browser'}
      else
        raise("#{device} is not a recognised device!")
    end
    driver = Selenium::WebDriver.for(:remote, desired_capabilities: caps, url: 'http://0.0.0.0:4723/wd/hub')
