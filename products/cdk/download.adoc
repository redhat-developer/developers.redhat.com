...
==== 1. Vagrant and virtualization platform (hypervisor)
...
* Microsoft Windows 7 and later, link://https://releases.hashicorp.com/vagrant/1.7.4/vagrant_1.7.4.msi[download the MSI universal installer].
* Apple Mac OS X, link:https://releases.hashicorp.com/vagrant/1.7.4/vagrant_1.7.4.dmg[down the DMG universal installer].
...
* Fedora 23, install the Vagrant packages included with Fedora.
* another Linux distribution, check your distribution's packages or download a `.deb` or `.rpm` package from link:https://vagrantup.com/downloads.html[vagrantup.com]


==== 2. Red Hat Enterprise Linux CDK Download

From the Red Hat CDK 2 download page you need to download two files:
* First, download link:#{site.download_manager_file_base_url}/cdk-2.0.0-beta4.zip[Red Hat Container Tools] to get the CDK components to be installed on your system.
...
[.callout-light]
*NOTE:*  In order to run the Red Hat Enterprise Linux Vagrant box, you must have signed the Developer Suite terms and conditions. If you have not done this already, log in to the Customer Portal with your Red Hat account and link:https://www.redhat.com/wapps/ugc[sign the Red Hat Enterprise Linux Developer Suite Terms & Conditions].


== Container images available from Red Hat 

Here are some of the container images available from Red Hat. See the link:https://access.redhat.com/search/#/container-images[Red Hat Registry] for a complete list, as well as other options available in beta versions.
