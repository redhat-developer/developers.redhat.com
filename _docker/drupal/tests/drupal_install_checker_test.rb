...
class DrupalInstallCheckerTest < Minitest::Test
  def test_settings_exists_when_true
    ...
    begin
      ...
      cut = DrupalInstallChecker.new(drupal_site, nil)
  ...
  def test_mysql_connect_when_true
    ...
    process_exec.expect :exec!, true, ['mysql', 'drupal']
    cut = DrupalInstallChecker.new(nil, process_exec)
  ...
  def test_tables_exists_when_true
    ...
    process_exec.expect :exec!, tables_to_expect, ['mysql', ['--execute=show tables', 'drupal']]
    cut = DrupalInstallChecker.new(nil, process_exec)
  ...
  def test_tables_exists_when_false
    process_exec = Minitest::Mock.new
    process_exec.expect :exec!, '', ['mysql', ['--execute=show tables', 'drupal']]
    cut = DrupalInstallChecker.new(nil, process_exec)
  ...
  def test_installed_when_true
    ...
    cut = DrupalInstallChecker.new(drupal_site, process_exec)
