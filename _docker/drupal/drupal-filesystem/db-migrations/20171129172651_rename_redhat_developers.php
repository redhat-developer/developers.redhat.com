...
class RenameRedhatDevelopers extends AbstractMigration
{
    ...
    public function up() {
        ...
        $this->execute('UPDATE lightning_node_field_data SET moderation_state = "published" WHERE moderation_state IS NULL AND status = 1');
        $this->execute('UPDATE lightning_node_field_data SET moderation_state = "draft" WHERE moderation_state IS NULL AND status = 0');
