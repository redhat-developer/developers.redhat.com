...
class TestOptions < Minitest::Test
  ...
  def test_acceptance_test_target_task
    ClimateControl.modify PARALLEL_TEST: 'true', RHD_JS_DRIVER: 'docker_chrome', RHD_DEFAULT_DRIVER: 'mechanize' do
      ...
      assert_equal(["--rm", "--service-ports",'-e','SELENIUM_HOST=http://selhub:4444/wd/hub', "awestruct", "bundle exec rake features HOST_TO_TEST=#{ENV['HOST_TO_TEST']} RHD_JS_DRIVER=#{ENV['RHD_DOCKER_DRIVER']}"], tasks[:acceptance_test_target_task])
    end
  end
