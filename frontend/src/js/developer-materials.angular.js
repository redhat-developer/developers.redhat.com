...
dcp.service('materialService', function($http, $q) {
    ...
    // - http://www.bennadel.com/blog/2616-aborting-ajax-requests-using-http-and-angularjs.htm
      ...
      // to call it always... (which is what we do now).
        ...
        this.deferred_ = $q.defer();
        promise = this.deferred_.promise;

    // query = decodeURIComponent(query);
        deferred = this.deferred_;
   ...
   */
    ...
    */
        this.removeTagItems = function(input) {
            var indexes = [], excludes;

            if ($.isArray(input) && input.length > 0) {
                excludes = app.dcp.excludeResourceTags;
...
dcp.filter('isPremium', ['helper', function(helper) {
    return function(url) {
        ...
        if (url) {
            return Boolean(url.match("access.redhat.com"));
        }
        return false;
...
dcp.filter('HHMMSS', ['helper', function(helper) {
    return function(sec_string) {
        ...
        sec_num = parseInt(sec_string, 10); // don't forget the second param
...
dcp.filter('formatName', ['helper', function(helper) {
    return function(value, scope){
        ...
        for (f in scope.data.availableFormats) {
            format = scope.data.availableFormats[f];
    ...
    ['$scope', 'materialService', '$rootScope', '$location', 'helper', 'dataFlowService',
        function($scope, materialService, $rootScope, $location, helper, dataFlowService) {
            ...
            */
            $scope.paginate = function(page) {
                ...
                startAt = page * $scope.pagination.size - $scope.pagination.size;
                endAt = page * $scope.pagination.size;
                pages = Math.ceil($scope.data.total / $scope.pagination.size);
            ...
            $scope.filter.toggleSelection = function toggleSelection(itemName, selectedArray) {
                ...
                idx = $scope.filters[selectedArray].indexOf(itemName);
            ...
            */
            ...
            $scope.filter.restore = function() {
                var filters, filterArr, labels, idx, parts,
                    d, now, ago, daysAgo, then;
                // restore and set
                $scope.data.skillNumber = 0;
                $scope.data.dateNumber = 0;
                ...
                if ($location.path().length > 0) {
                    ...
                    filters = deparam($location.path().slice(1));
                    ...
                    if ($scope.filters.level) {
                        labels = ["All", "Beginner", "Intermediate", "Advanced"];
                        idx = labels.indexOf($scope.filters.level);
                        if (idx>=0) {
      ...
      // restore date slider to closest match
                    if ($scope.filters.publish_date) {
                        parts = $scope.filters.publish_date.split('-'); // YYYY MM DD
                        d = new Date(parts[0], parts[1], parts[2]); // Year, month date
                        now = new Date().getTime();
                        ago = now - d;
                        daysAgo = Math.floor(ago / 1000 / 60 / 60 / 24);
                    ...
                    now = new Date().getTime();
                    then = window.localStorage[$scope.data.pageType + '-filtersTimeStamp'] || 0;
