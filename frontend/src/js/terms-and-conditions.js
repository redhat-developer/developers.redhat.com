"use strict";
/* global app, digitalData, sendCustomEvent */
/*
...
app.termsAndConditions = {
    ...
    download: function () {
        var tcUser = $.encoder.canonicalize(app.termsAndConditions.urlParam('tcUser')),
            ...
            whenSigned = app.termsAndConditions.urlParam('tcWhenSigned') || '',
            tcWhenSigned = $.encoder.canonicalize(whenSigned).replace(/\+/g, ' '),
            tcEndsIn = $.encoder.canonicalize(app.termsAndConditions.urlParam('tcEndsIn')),
            tcDownloadURL = $.encoder.canonicalize(app.termsAndConditions.urlParam('tcDownloadURL')),
            tcDownloadFileName = $.encoder.canonicalize(app.termsAndConditions.urlParam('tcDownloadFileName')),
            product = $.encoder.canonicalize(app.termsAndConditions.urlParam('p')),
            tcProduct = $.encoder.canonicalize(product) || '',
            ddDownloadEvent;
    ...
    banner: function() {
        app.dcp.authStatus().done(function(data) {
            ...
            if (data.authenticated) {
                ...
                tac = document.createElement('script'); tac.type = 'text/javascript'; tac.async = true;
                ...
                s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(tac, s);
            }
        });
    }
