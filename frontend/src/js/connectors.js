...
app.connectors = {
    ...
    format: function (data, container, thumbnailSize) {
        var hits = data.responses ? data.responses[0].hits.hits : data.hits.hits,
            html = "",
...
$(function () {
    var targetProductFilter = $('[data-target-product]').data('target-product'),
        orderBy = $('[data-order-by]').data('order-by'),
        connectorResults = $('.connector-results'),
        featuredConnectorIds = $('.featured-connector-ids'),
        featuredIds;
    ...
    if (featuredConnectorIds.length) {
        featuredIds = JSON.parse(featuredConnectorIds.text());
