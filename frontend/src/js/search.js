"use strict";
/* global angular, app, digitalData, sendCustomEvent, deparam */
...
search.filter('wordcount', function() {
    return function(description) {
        var wordCount, pieces;
        description = description || '';
        wordCount = 50;
        pieces = description.split(' ').slice(0, wordCount);
        return pieces.join(' ') + (pieces.length >= wordCount ? '…' : '');
    }
});
 ...
 */
search.filter('stackDate', function() {
    return function(result) {
...
function searchCtrlFunc($scope, $window, searchService) {
    var isStackOverflow = /stack-overflow/.test(window.location.href)
        || /help/.test(window.location.href);
    var isSearch = Boolean(window.location.href.match(/\/search/));
    var isResources = Boolean(window.location.href.match(/\/resources/));
    var searchTerm = window.location.search.split('=');
    var isFirstSearch = true;
    var q = '', selectedProduct;
  ...
  // Stack Oveflow Page Specifics
    if (isStackOverflow && searchTerm) {
        selectedProduct = $window.location.hash.replace('#!q=', '');
  ...
  */
    ...
    $scope.updateSearch = function() {
        var params, searchPage, tags, product, blog_tags;
        $scope.loading = true;
    ...
    // if Sort by filter selection changed from New First to Most Recent, remove newFirst flag
        ...
        if (isResources && $scope.userFilters) {
            ...
            if ($scope.params.project && $scope.params.sys_type.includes("blogpost")) {
                ...
                if (app.products[product].buzz_tags !== '_none') {
                    blog_tags = app.products[product].buzz_tags;
        ...
        searchService.getSearchResults(params).then(function(data) {
            var types, ddSearchEvent;
            if (!window.digitalData) {
                window.digitalData = {page: {listing: {}, category: {primaryCategory: ''}}, event: []};
            }
            types = {
                video: 'Video',
                blogpost: 'Blog Post',
            ...
            ddSearchEvent = {
                eventInfo: {
                    ...
                    listing: {
                        ...
                        searchType: window.digitalData.page.category.primaryCategory || "",
    ...
    // if URL contains filter params, adds them to $scope.params
        if (window.location.hash) {
            ...
            hashFilters = window.location.hash.replace('#!', '');
  ...
  */
    ...
    $scope.toggleSelection = function toggleSelection(event) {
        var checkbox = event.target,
            topicNames = checkbox.value.split(' '),
            filterArr;
