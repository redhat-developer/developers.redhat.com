...
Array.prototype.sortJsonArrayByProperty = function sortJsonArrayByProperty(prop){
    ...
    this.sort(function(a, b){
        ...
        out = a > b ? Number(direct) : 0;
        return a < b ? -direct : out;
    });
};
    ...
    $('[data-set]').each(function(i, el){
        ...
        for (key in data) {
            if (key.match(/^set[A-Z]/g)) {
                attr = key.replace('set', '').replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
...
if (typeof Object.assign !== 'function') {
    (function () {
        Object.assign = function (target) {
            var output, source, index, nextKey;
