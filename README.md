...
Add the host `docker` to your `/etc/hosts` file. If you are building on Linux, set the IP address to `127.0.0.1`.
...
To run the acceptance tests (where HOST_TO_TEST is the host you are interested in:
      ...
      bundle exec ./control.rb -e drupal-dev --acceptance_test_target[=HOST_TO_TEST]

To run the acceptance tests against the locally running docker stack:

      bundle exec ./control.rb -e drupal-dev --acceptance_test_docker
